...
static void makepartie1(t_list_inst *list_ins, int local)
{
  ...
  wr.pop_target = get_gadget_by_addr(tab_x8632, ret_addr_makecodefunc(list_ins, reg_stack));
  wr.pop_data = get_gadget_by_addr(tab_x8632, ret_addr_makecodefunc(list_ins, reg_binsh));

  wr.zero_data = get_gadget_by_addr(tab_x8632, ret_addr_makecodefunc(list_ins, instr_xor));
  ...
  if (local)
    {
      ...
      sc_print_argv((const char * const *)&argv[0], &wr, 0, TRUE, 4, &argv_start, &envp_start);
    }
  else
    {
      char opts[9] = {0};
...
*/
  ...
  gad1      = get_gadget_since_addr_att(tab_x8632, addr_gad1);
  addr_gad2 = ret_addr_makecodefunc(list_ins, "mov (%e?x),%e?x");
  gad2      = get_gadget_since_addr_att(tab_x8632, addr_gad2);
  addr_gad3 = ret_addr_makecodefunc(list_ins, "mov %e?x,%e?x");
  gad3      = get_gadget_since_addr_att(tab_x8632, addr_gad3);
  addr_gad4 = ret_addr_makecodefunc(list_ins, "mov %e?x,(%e?x)");
  gad4      = get_gadget_since_addr_att(tab_x8632, addr_gad4);
