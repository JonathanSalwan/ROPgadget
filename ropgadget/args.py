...
""",
  ...
  """,
        ...
        parser.add_argument(
            ...
            "--version",
            action="store_true",
            help="Display the ropgadget's version",
        )
        parser.add_argument(
            "-c",
            "--checkUpdate",
        ...
        parser.add_argument(
            ...
            type=str,
            metavar="<binary>",
            help="Specify a binary filename to analyze",
        )
        parser.add_argument(
            "--opcode",
            type=str,
            metavar="<opcodes>",
            help="Search opcode in executable segment",
        )
        parser.add_argument(
            "--string",
            type=str,
            metavar="<string>",
            help="Search string in readable segment",
        )
        parser.add_argument(
            "--memstr",
            type=str,
            metavar="<string>",
            help="Search each byte in all readable segment",
        )
        parser.add_argument(
            "--depth",
            type=int,
        ...
        parser.add_argument(
            "--only", type=str, metavar="<key>", help="Only show specific instructions"
        )
        parser.add_argument(
            "--filter", type=str, metavar="<key>", help="Suppress specific mnemonics"
        )
        parser.add_argument(
            "--range",
            type=str,
        ...
        parser.add_argument(
            ...
            type=str,
            metavar="<arch>",
            help="Specify an arch for a raw file x86|arm|arm64|sparc|mips|ppc|riscv",
        )
        parser.add_argument(
            "--rawMode",
            type=str,
            metavar="<mode>",
            help="Specify a mode for a raw file 32|64|arm|thumb",
        )
        parser.add_argument(
            "--rawEndian",
            type=str,
            metavar="<endian>",
            help="Specify an endianness for a raw file little|big",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            metavar="<hexaddr>",
            help="Specify an offset for gadget addresses",
        )
        parser.add_argument(
            "--ropchain", action="store_true", help="Enable the ROP chain generation"
        )
        parser.add_argument(
            "--thumb",
            action="store_true",
            help="Use the thumb mode for the search engine (ARM only)",
        )
        parser.add_argument(
            "--console",
            action="store_true",
            help="Use an interactive console for search engine",
        )
        parser.add_argument(
            "--norop", action="store_true", help="Disable ROP search engine"
        )
        parser.add_argument(
            "--nojop", action="store_true", help="Disable JOP search engine"
        )
        parser.add_argument(
            "--callPreceded",
            action="store_true",
            help="Only show gadgets which are call-preceded",
        )
        parser.add_argument(
            "--nosys", action="store_true", help="Disable SYS search engine"
        )
        parser.add_argument(
            "--multibr", action="store_true", help="Enable multiple branch gadgets"
        )
        parser.add_argument(
            "--all",
            action="store_true",
            help="Disables the removal of duplicate gadgets",
        )
        parser.add_argument(
            "--noinstr",
            action="store_true",
            help="Disable the gadget instructions console printing",
        )
        parser.add_argument(
            "--dump", action="store_true", help="Outputs the gadget bytes"
        )
        parser.add_argument(
            "--silent",
            action="store_true",
            help="Disables printing of gadgets during analysis",
        )
        parser.add_argument(
            "--align", type=int, help="Align gadgets addresses (in bytes)"
        )
        parser.add_argument(
            "--mipsrop",
            type=str,
            metavar="<rtype>",
            help="MIPS useful gadgets finder stackfinder|system|tails|lia0|registers",
        )
        ...
        if self.__args.noinstr and self.__args.only:
            raise ValueError(
                "[Error] --noinstr and --only=<key> can't be used together"
            )
        ...
        if (
            self.__args.rawArch
            and not self.__args.rawEndian
            and self.__args.rawArch != "x86"
        ):
        ...
        if self.__args.version:
            ...
            not custom_arguments_provided
            and not self.__args.binary
            and not self.__args.console
        ):
            raise ValueError(
            ...
            try:
                ...
                raise ValueError(
                    "[Error] A range must be set in hexadecimal. Ex: 0x08041000-0x08042000"
                )
            if rangeS > rangeE:
                raise ValueError(
                    "[Error] The start value must be greater than end value"
                )
