...
class ELF(object):
    ...
    def __setShdr(self):
        ...
        if self.__ElfHeader.e_shstrndx != 0:
            string_table = bytes(
                self.__binary[(self.__shdr_l[self.__ElfHeader.e_shstrndx].sh_offset) :]
            )
            for i in range(shdr_num):
                self.__shdr_l[i].str_name = (
                    string_table[self.__shdr_l[i].sh_name :]
                    .split(b"\x00")[0]
                    .decode("utf8")
                )
    ...
    def getArch(self):
        if (
            self.__ElfHeader.e_machine == ELFFlags.EM_386
            or self.__ElfHeader.e_machine == ELFFlags.EM_X86_64
        ):
            return CS_ARCH_X86
            ...
            self.__ElfHeader.e_machine == ELFFlags.EM_PowerPC
            or self.__ElfHeader.e_machine == ELFFlags.EM_PPC64
        ):
            return CS_ARCH_PPC
