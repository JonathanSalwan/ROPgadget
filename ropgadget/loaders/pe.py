...
class PE(object):
    ...
    def __getPEOffset(self):
        ...
        if self.__binary[self.__PEOffset : self.__PEOffset + 4] != unhexlify(
            b"50450000"
        ):
    ...
    def __parseOptHeader(self):
        PEoptHeader = self.__binary[
            self.__PEOffset
            + 24 : self.__PEOffset
            + 24
            + self.__IMAGE_FILE_HEADER.SizeOfOptionalHeader
        ]

        if (
            unpack("<H", bytes(PEoptHeader[0:2]))[0]
            == PEFlags.IMAGE_NT_OPTIONAL_HDR32_MAGIC
        ):
            self.__IMAGE_OPTIONAL_HEADER = IMAGE_OPTIONAL_HEADER.from_buffer_copy(
                PEoptHeader
            )
            ...
            self.__IMAGE_OPTIONAL_HEADER = IMAGE_OPTIONAL_HEADER64.from_buffer_copy(
                PEoptHeader
            )
    ...
    def __parseSections(self):
        baseSections = (
            self.__PEOffset + 24 + self.__IMAGE_FILE_HEADER.SizeOfOptionalHeader
        )
        sizeSections = self.__IMAGE_FILE_HEADER.NumberOfSections * sizeof(
    ...
    def getEntryPoint(self):
        return (
            ...
            + self.__IMAGE_OPTIONAL_HEADER.AddressOfEntryPoint
        )
    ...
    def getArch(self):
        if (
            self.__IMAGE_FILE_HEADER.Machine == PEFlags.IMAGE_MACHINE_INTEL_386
            or self.__IMAGE_FILE_HEADER.Machine == PEFlags.IMAGE_MACHINE_AMD_8664
        ):
            return CS_ARCH_X86
        if (
            self.__IMAGE_FILE_HEADER.Machine == PEFlags.IMAGE_FILE_MACHINE_ARM
            or self.__IMAGE_FILE_HEADER.Machine == PEFlags.IMAGE_FILE_MACHINE_ARMV7
        ):
            return CS_ARCH_ARM
    ...
    def getArchMode(self):
        if self.__IMAGE_OPTIONAL_HEADER.Magic == PEFlags.IMAGE_NT_OPTIONAL_HDR32_MAGIC:
            ...
            self.__IMAGE_OPTIONAL_HEADER.Magic == PEFlags.IMAGE_NT_OPTIONAL_HDR64_MAGIC
        ):
            return CS_MODE_64
