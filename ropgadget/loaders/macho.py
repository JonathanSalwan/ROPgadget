...
class MACHO(object):
    ...
    def getExecSections(self):
        ...
        for section in self.__sections_l:
            if (
                section.flags & MACHOFlags.S_ATTR_SOME_INSTRUCTIONS
    ...
    def getDataSections(self):
        ...
        for section in self.__sections_l:
            if (
                not section.flags & MACHOFlags.S_ATTR_SOME_INSTRUCTIONS
    ...
    def getArch(self):
        if (
            self.__machHeader.cputype == MACHOFlags.CPU_TYPE_I386
            or self.__machHeader.cputype == MACHOFlags.CPU_TYPE_X86_64
        ):
            return CS_ARCH_X86
        ...
        if self.__machHeader.cputype == MACHOFlags.CPU_TYPE_MIPS:
            return CS_ARCH_MIPS
        if (
            self.__machHeader.cputype == MACHOFlags.CPU_TYPE_POWERPC
            or self.__machHeader.cputype == MACHOFlags.CPU_TYPE_POWERPC64
        ):
            return CS_ARCH_PPC
