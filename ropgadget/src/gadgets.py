...
class Gadgets(object):
    ...
    def __passCleanX86(self, decodes):
        ...
        if decodes[-1][2] not in br:
            return True
        if not self.__options.multibr and any(
            mnemonic in br for _, _, mnemonic, _ in decodes[:-1]
        ):
            return True
            ...
            for ref in allRefRet:
                ...
                for i in range(self.__options.depth):
                    ...
                    if (sec_vaddr + start) % gad_align == 0:
                        ...
                        if (
                            sum(size for _, size, _, _ in decodes)
                            != i * gad_align + gad_size
                        ):
    ...
    def addROPGadgets(self, section):
        ...
        if arch == CS_ARCH_X86:
            ...
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [[b"\xd6\x5f\x03\xc0", 4, 4]]  # ret
            else:
                gadgets = [[b"\xc0\x03\x5f\xd6", 4, 4]]  # ret
        ...
        if gadgets:
            return self.__gadgetsFinding(
                section, gadgets, arch, arch_mode + arch_endian
            )
        return gadgets
    ...
    def addJOPGadgets(self, section):
        ...
        if arch == CS_ARCH_X86:
            ...
            if arch_mode == CS_MODE_64:
                gadgets += [
                    (b"\x41" + op, size + 1, align) for (op, size, align) in gadgets
                ]
            ...
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [
                    [
                        b"\x00[\x40\x60\x80\xa0\xc0\xe0]\xf8\x09[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $v[0-1]|$a[0-3]
                    [
                        b"[\x01\x02][\x00\x20\x40\x60\x80\xa0\xc0\xe0]\xf8\x09[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $t[0-7]|$s[0-7]
                    [
                        b"\x03[\x00\x20\xc0\xe0]\xf8\x09[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $t[8-9]|$s8|$ra
                    [
                        b"\x00[\x40\x60\x80\xa0\xc0\xe0]\x00\x08[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $v[0-1]|$a[0-3]
                    [
                        b"[\x01\x02][\x00\x20\x40\x60\x80\xa0\xc0\xe0]\x00\x08[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $t[0-7]|$s[0-7]
                    [
                        b"\x03[\x00\x20\xc0\xe0]\x00\x08[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $t[8-9]|$s8|$ra
                ...
                gadgets = [
                    [
                        b"\x09\xf8[\x40\x60\x80\xa0\xc0\xe0]\x00[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $v[0-1]|$a[0-3]
                    [
                        b"\x09\xf8[\x00\x20\x40\x60\x80\xa0\xc0\xe0][\x01\x02][\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $t[0-7]|$s[0-7]
                    [
                        b"\x09\xf8[\x00\x20\xc0\xe0]\x03[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jalr $t[8-9]|$s8|$ra
                    [
                        b"\x08\x00[\x40\x60\x80\xa0\xc0\xe0]\x00[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $v[0-1]|$a[0-3]
                    [
                        b"\x08\x00[\x00\x20\x40\x60\x80\xa0\xc0\xe0][\x01\x02][\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $t[0-7]|$s[0-7]
                    [
                        b"\x08\x00[\x00\x20\xc0\xe0]\x03[\x00-\xff]{4}",
                        8,
                        4,
                    ],  # jr $t[8-9]|$s8|$ra
            ...
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [[b"\x48[\x00-\xff]{3}", 4, 4]]  # bl
            else:
                gadgets = [[b"[\x00-\xff]{3}\x48", 4, 4]]  # bl

        elif arch == CS_ARCH_SPARC:
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [[b"\x81\xc0[\x00\x40\x80\xc0]{1}\x00", 4, 4]]  # jmp %g[0-3]
            else:
                gadgets = [[b"\x00[\x00\x40\x80\xc0]{1}\xc0\x81", 4, 4]]  # jmp %g[0-3]
            arch_mode = 0
        elif arch == CS_ARCH_ARM64:
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [
                    [
                        b"\xd6[\x1f\x5f]{1}[\x00-\x03]{1}[\x00\x20\x40\x60\x80\xa0\xc0\xe0]{1}",
                        4,
                        4,
                    ],  # br reg
                    [
                        b"\xd6\?[\x00-\x03]{1}[\x00\x20\x40\x60\x80\xa0\xc0\xe0]{1}",
                        4,
                        4,
                    ],  # blr reg # noqa: W605 # FIXME: \?
                ]
            else:
                gadgets = [
                    [
                        b"[\x00\x20\x40\x60\x80\xa0\xc0\xe0]{1}[\x00-\x03]{1}[\x1f\x5f]{1}\xd6",
                        4,
                        4,
                    ],  # br reg
                    [
                        b"[\x00\x20\x40\x60\x80\xa0\xc0\xe0]{1}[\x00-\x03]{1}\?\xd6",
                        4,
                        4,
                    ],  # blr reg # noqa: W605 # FIXME: \?
                ]
            ...
            if self.__options.thumb or self.__options.rawMode == "thumb":
                if arch_endian == CS_MODE_BIG_ENDIAN:
                    gadgets = [
                        [
                            b"\x47[\x00\x08\x10\x18\x20\x28\x30\x38\x40\x48\x70]{1}",
                            2,
                            2,
                        ],  # bx   reg
                        [
                            b"\x47[\x80\x88\x90\x98\xa0\xa8\xb0\xb8\xc0\xc8\xf0]{1}",
                            2,
                            2,
                        ],  # blx  reg
                    ...
                    gadgets = [
                        [
                            b"[\x00\x08\x10\x18\x20\x28\x30\x38\x40\x48\x70]{1}\x47",
                            2,
                            2,
                        ],  # bx   reg
                        [
                            b"[\x80\x88\x90\x98\xa0\xa8\xb0\xb8\xc0\xc8\xf0]{1}\x47",
                            2,
                            2,
                        ],  # blx  reg
            ...
            if arch_endian == CS_MODE_BIG_ENDIAN:
                ...
                gadgets = [
                    ...
                    [
                        ...
                        4,
                        1,
                    ],  # jalr/j/jal register, offset
                    [
                        b"[\x63\xe3][\x00-\xff][\x00-\xff]{2}",
                        4,
                        1,
    ...
    def addSYSGadgets(self, section):
        ...
        if arch == CS_ARCH_X86:
            ...
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [[b"\x00\x00\x00\x0c", 4, 4]]  # syscall
            else:
                gadgets = [[b"\x0c\x00\x00\x00", 4, 4]]  # syscall
        elif arch == CS_ARCH_PPC:
            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [
            ...
            if self.__options.thumb or self.__options.rawMode == "thumb":
                gadgets = [[b"\x00-\xff]{1}\xef", 2, 2]]  # FIXME: svc
                arch_mode = CS_MODE_THUMB
            else:
                gadgets = [[b"\x00-\xff]{3}\xef", 4, 4]]  # FIXME: svc
                arch_mode = CS_MODE_ARM
        elif arch == CS_ARCH_RISCV:

            if arch_endian == CS_MODE_BIG_ENDIAN:
                gadgets = [[b"\x00\x00\x00\x73", 4, 2]]  # syscall
            else:
                gadgets = [[b"\x73\x00\x00\x00", 4, 2]]  # syscall
